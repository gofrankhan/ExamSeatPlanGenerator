...
if(isset($_POST['form_course'])) {
	...
	try {
		...
		$statement1 = $db->prepare("update tb_block_course set course_id=?, section=?, batch_no=? where course_id=? and section=? and batch_no=?");
		$statement1->execute(array($_POST['course_id'], $_POST['section'], $_POST['batch_no'], $course_id,$section,$batch_no));
			
		$statement = $db->prepare("update tb_course set course_title=?,course_id=?,section=?,std_reg=?,tech_name=?,batch_no=? where id=?");
		$statement->execute(array($_POST['course_title'],$_POST['course_id'],$_POST['section'],$_POST['std_reg'],$_POST['tech_name'],$_POST['batch_no'],$id));
		
		$success_message = 'Data has been inserted successfully.';
		header("location: course_wise_registration.php?search=$course_id");
...
<html lang="en">
  <head>
    ...
    <title>Update Course::Exam Hall Manager</title>
  ...
  <div class="container-fluid">
				...
				<div class="container-fluid">
					...
					<div class="collapse navbar-collapse" id="bs-sidebar-navbar-collapse-1">
					  <ul class="nav navbar-nav">
						...
						<li class="active"><a href="insert_course_info.php">Update Courses<span style="font-size:16px;" class="pull-right hidden-xs showopacity glyphicon"></span></a></li>
	...
	<!-- ENDOF LEFT LIST-->
			...
			<?php
				...
				$statement = $db->prepare("select * from tb_course where id=?");
				$statement->execute(array($id));
				$result = $statement->fetchAll(PDO::FETCH_ASSOC);
				foreach($result as $row)
				{
		...
		?>
			...
			<div class="panel-heading">Update Course Information</div>
			...
			<form action="" method="post">
				...
				<div class="form-group row">
				  ...
				  <div class="col-xs-8">
					<input class="form-control" type="text" name="section" value="<?php echo $section?>" id="example-text-input">
				  </div>
				</div>
				<div class="form-group row">
				  <label for="example-text-input" class="col-xs-4 col-form-label">Registered Students</label>
				  <div class="col-xs-8">
					<input class="form-control" type="number" name="std_reg" value="<?php echo $std_reg?>" id="example-text-input">
				  </div>
				</div>
				<div class="form-group row">
				  <label for="example-text-input" class="col-xs-4 col-form-label">Teacher's Init</label>
				  <div class="col-xs-8">
					<input class="form-control" type="text" name="tech_name" value="<?php echo $tech_name?>" id="example-text-input">
				  </div>
				</div>
				<div class="form-group row">
				...
				<button for="example-time-input" type="submit" name="form_course" class="col-xs-3 btn btn-success">Update</button>
				</div>
				</form>
			</div>
		</div>
