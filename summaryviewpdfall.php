...
if(isset($_POST['all'])) {
	$statement = $db->prepare("select distinct date, time from tb_room_column_status order by date, time asc");
	$statement->execute();
	...
	$statement1 = $db->prepare("select * from tb_routine where start_time=?");
	$statement1->execute(array($time));
	...
	$statement2 = $db->prepare("select * from tb_exam_info where id=?");
