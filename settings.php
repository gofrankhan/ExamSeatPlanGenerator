...
<?php 
if(isset($_POST['form_block_room'])) {
	try{
		$s1 = $db->prepare("update tb_block_room  set status=?");
		$s1->execute(array("0"));

	if(!empty($_POST['checkbox'])) {
    foreach($_POST['checkbox'] as $room_no) {
			$statement1 = $db->prepare("update tb_block_room set status=? where room_no=?");
					...
					<div class="collapse navbar-collapse" id="bs-sidebar-navbar-collapse-1">
					  <ul class="nav navbar-nav">
					   <li ><a href="exam_registration.php">Exam Registration<span style="font-size:16px;" class="pull-right hidden-xs showopacity glyphicon"></span></a></li>
						<li class="active"><a href="settings.php">Block Room<span style="font-size:16px;" class="pull-right hidden-xs showopacity glyphicon"></span></a></li>
			...
			<?php
				...
				<?php
					...
					$room_no = $row1['room_no'];
						$s1 = $db->prepare("select status from tb_block_room  where room_no=?");
				...
				<div class="form-group">
					<button for="example-time-input" type="submit" name="form_block_room" class="col-xs-3 btn btn-success">Save</button>
				</div>
				</form>
			</div>
