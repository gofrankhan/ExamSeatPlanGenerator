	...
	<!-- lEFT LIST START -->
			...
			<div class="col-md-9 col-sm-4">
				...
				if(isset($_POST['search'])) {
						...
						$spanCountCourse = 0;
						$spanCountSection = 0;
						$spanArrayCourse = array();
						$spanArraySection = array();
							...
							foreach($r_course as $row_course)
							 {
								 ...
								 $spanArraySection[] = $spanCountSection;
							 }
							 $spanArrayCourse[] = $spanCountCourse;
						}
				?>
				...
				<table class="table table-bordered">
					...
					<tbody>
						<tr class="something">
							<th class="col-md-1" rowspan = "<?php echo $spanCountCSE;?>">CSE</th>
							<?php
							$total = 0;
							$spanCount = 0;
							$indexCourse = 0;
							...
							foreach($r_time as $row_time)
							{
								...
								$batch_no = $row_time['batch_no'];
								?>
							<td class="col-md-1" rowspan = "<?php echo $spanArrayCourse[$indexCourse];?>"><?php echo $course_id ;?></td>
							<td class="col-md-2" rowspan = "<?php echo $spanArrayCourse[$indexCourse++];?>"><?php echo $course_title ;?></td>
							<?php 
							 $st_course = $db->prepare("select * from tb_course where course_id=? and batch_no=? order by section asc");
							 $st_course->execute(array($course_id,$batch_no));
							 $r_course = $st_course->fetchAll(PDO::FETCH_ASSOC);
							 foreach($r_course as $row_course)
							 {
								 ...
								 $spaner = 0;
							 ?> 
							<td class="col-md-1" rowspan = "<?php echo $spanArraySection[$indexSection];?>"><?php echo $tech_int; ?></td>
							<td class="col-md-1" rowspan = "<?php echo $spanArraySection[$indexSection];?>"><?php echo $section; ?></td>
							<?php 
								$st_details = $db->prepare("select room_no, count from tb_details_count where course_id=? and batch_no=? and section=? and date=? and time=? order by section asc");
								$st_details->execute(array($course_id,$batch_no,$section,$date,$start_time));
								$r_details = $st_details->fetchAll(PDO::FETCH_ASSOC);
								...
								<?php if($spaner==0){?>
								<td class="col-md-1" rowspan = "<?php echo $spanArraySection[$indexSection++];?>"><?php echo $t_count;?></td>
								<?php $spaner++;} ?>
								</tr>
								<?php
								}
							...
							?>
							<tr><th class="col-md-1" colspan = "7">Total</th>
