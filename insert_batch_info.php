...
if(isset($_POST['form_batch'])) {
	...
	try {
		...
		for($i = 0; $i < $_POST['section'];$i++){
			$statement = $db->prepare("insert into tb_course (course_title,course_id,section,batch_no) values(?,?,?,?)");
			if($_POST['even'] == 'yes')
				$statement->execute(array($_POST['course_title'],$_POST['course_id'],"EVE-".chr($i+65),$_POST['batch_no']));
			else
				$statement->execute(array($_POST['course_title'],$_POST['course_id'],chr($i+65),$_POST['batch_no']));
			
			$statement = $db->prepare("insert into tb_block_course (course_id, section, batch_no, status) values(?,?,?,?)");
			if($_POST['even'] == 'yes')
				$statement->execute(array($_POST['course_id'],"EVE-".chr($i+65),$_POST['batch_no'],'0'));
			else
				$statement->execute(array($_POST['course_id'],chr($i+65),$_POST['batch_no'],'0'));
		}
				...
				<div class="container-fluid">
					...
					<div class="collapse navbar-collapse" id="bs-sidebar-navbar-collapse-1">
					  <ul class="nav navbar-nav">
						...
						<li class="active"> <a href="insert_batch_info.php">Insert Batch <span style="font-size:16px;" class="pull-right hidden-xs showopacity glyphicon "></span></a>  
	...
	<!-- ENDOF LEFT LIST-->
			...
			<form action="" method="post">
				...
				<div class="form-group row">
				  ...
				  <div class="col-xs-8">
					<input class="form-control" type="number" name="section" value="1" id="example-text-input">
				  </div>
				</div>
				<div class="form-group row">
				...
				<div class="form-group row">
				  ...
				  <div class="col-xs-8">
					<label><input class="form-control" type="checkbox" name="even" value="yes" id="example-text-input">Yes</label>
				  </div>
				</div>
				</div>
				</div>
				<div class="form-group">
				<button for="example-time-input" type="submit" name="form_batch" class="col-xs-3 btn btn-success">Save</button>
				</div>
				</form>
			</div>
		</div>
